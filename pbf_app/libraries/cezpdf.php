...
//  $y-=$gap-$decender;
  ...
  while ($ok==0){
    foreach($cols as $colName=>$colHeading){
      ...
      $this->ezText('<b>'.$colHeading.'</b>',$size,array('aleft'=> $pos[$colName],'aright'=>($maxWidth[$colName]+$pos[$colName]),'justification'=>$justification));
      $dy = $y-$this->y;
      if ($dy>$mx){
        $mx=$dy;
      }
...
//        $x1=$x1+$dm;
      ...
      foreach($cols as $colName=>$colTitle){
        ...
        foreach ($lines as $line){
          $line = utf8_decode($this->ezProcessText($line));
          $start=1;
...
// ---------- the function ezImage is reproduced with minor alteration playing arround the padding in order to align 3 odometers on A4 pdf page

function ezIfoto($image,$pad = 0,$width = 0,$resize = 'full',$just = 'center',$border = ''){
	...
	if ($type == "jpeg"){
	$this->addJpegFromFile($image,$this->ez['leftMargin'] + $pad + $offset, $this->y + $this->getFontHeight($this->ez['fontSize']) - $height,$width);
	}

	if ($type == "png"){
	$this->addPngFromFile($image,$this->ez['leftMargin'] + $pad + $offset, $this->y + $this->getFontHeight($this->ez['fontSize']) - $height,$width);
	}
	//draw border
	if ($border != '')
	{
...
function ezImage($image,$pad = 5,$width = 0,$resize = 'full',$just = 'center',$border = ''){
	...
	switch ($imageInfo[2]){
		...
		$new_image = dirname($image).'/aa_temp_image_detete_now.jpg';
		$image_identifier = imagecreatefrompng($image);
		$bg = imagecreatetruecolor(imagesx($image_identifier), imagesy($image_identifier));
		imagefill($bg, 0, 0, imagecolorallocate($bg, 255, 255, 255));
		imagealphablending($bg, TRUE);
		
		imagecopy($bg, $image_identifier, 0, 0, 0, 0, imagesx($image_identifier), imagesy($image_identifier));
		imagedestroy($image_identifier);
		imagejpeg($bg, $new_image, 60);// the 50 is to set the quality, 0 = worst-smaller file, 100 = better-bigger file 
		imagedestroy($bg);
		$this->ezImage($new_image,$pad,$width,$resize,$just,$border);
		
		//$type = "jpeg"; // was $type = "png" but hacked by AA;
