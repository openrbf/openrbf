<?php
	function quaterly_invoice_reg($params,$invoice_params = null)
		{
		...
		$sql="SELECT budget_value FROM pbf_budget WHERE entity_id= ".$params['entity_id']." AND budget_year=".$params['datafile_year'];
		$rw_subsides = $ci->db->query($sql)->row_array();
		if($rw_subsides){
			$subsides=$rw_subsides['budget_value'] /4 ;
		}else{
			$subsides=0;
		}
		...
		$sql="SELECT (datafile_total) AS quality FROM pbf_datafile  WHERE  (filetype_id IN (17,18,19,20,21)) AND datafile_quarter=".$params['datafile_quarter']." AND datafile_year=".$params['datafile_year']."  AND pbf_datafile.entity_id = ".$params['entity_id'];
		...
		*{
		...
		if($entity_info['entity_type']!=11 AND $entity_info['entity_type']!=12){
			$ci->dpdf->content .= utf8_decode('
			<tr>
				<td><strong>REGION: </strong>'.$entity_info['parent_geozone_name'].'</td>
			</tr>');
		}
		if($entity_info['entity_type']==10){ 
			$ci->dpdf->content .= utf8_decode('
			<tr>
		...
		$ci->dpdf->content .= utf8_decode('
		<tr>
			<td><strong>ENTITÉ: </strong>'.$entity_info['entity_name'].'</td>
		</tr>
		<tr><td><strong>PERIODE:</strong> '.$ci->lang->line('app_month_'.$months['0']).' - '.$ci->lang->line('app_month_'.$months['2']).' , '.$params['datafile_year'].'</td>
  
		</tr></table></p><br/><br/>');
  
		$ci->dpdf->content .= utf8_decode('<div style="padding:3px; border:0px solid black;"><h1 style="text-align:center ;  border-bottom: solid black 0px; padding:5px; margin:auto;font-size:17px;">Facture subsides trimestrielle</h1></div><br/><br/>');
  	
			
		$ci->dpdf->content .=utf8_decode('<table class="tb_contenu" style="width:80%; margin:auto;">
		<tr>
			<th class="gauche">Score de l\'évaluation de la performance (%)</th><td class="droite">'.number_format($quality,2).'</td>
		</tr>
		
		<tr>
			<th class="gauche">Subsides trimestriels disponibles</th><td class="droite">'.number_format($subsides).'</td>
		</tr>
		
		<tr>
			<th class="gauche">TOTAL A PAYER</th><th class="droite">'.number_format($prime).'</th>
		</tr>
		...
		$ci->dpdf->content .=utf8_decode('<br/><br/><p>Arrêté la présente facture pour  <b>'.$ci->lang->line('app_quarter_'.$params['datafile_quarter']).' - '.$params['datafile_year'].' </b> pour l\'entité <b>'.$entity_info['entity_name'].'</b> à un montant de <b>'.$ci->numbers_words->toWords($prime,'fr').' FCFA </b></p>');
