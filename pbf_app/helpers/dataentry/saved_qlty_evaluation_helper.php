...
'<script type="text/javascript">$(document).ready(function() {$("[id*=datafile_total]").blur(function(){$(this).parseNumber({format:"#,###.00", locale:"us"});$(this).formatNumber({format:"#,###.00", locale:"us"});});$("[id*=indicator_verified_value_]").blur(function(){var row_nbr = this.id.split("_");var str_len = row_nbr.length;row_nbr = row_nbr[parseInt(row_nbr.length)-1];var attrib_tot = 0;var available_tot = 0;var attrib = $("#indicator_verified_value_"+row_nbr).val().replace(/,/g,"");var available = $("#indicator_tarif_"+row_nbr).val().replace(/,/g,"");if(parseFloat(attrib) > parseFloat(available)){$("#indicator_verified_value_"+row_nbr).val(0);$("#indicator_montant_"+row_nbr).val(0);}else if(isNaN(attrib) || attrib=="-"){$("#indicator_verified_value_"+row_nbr).val("-");$("#indicator_montant_"+row_nbr).val("-");$("#available_tot").val(parseFloat($("#available_tot").val().replace(/,/g,""))-parseFloat($("#indicator_tarif_"+row_nbr).val().replace(/,/g,"")));}else{$("#indicator_montant_"+row_nbr).val(Math.round(((attrib/available)*100)*100)/100);$(this).parseNumber({format:"#,###.00", locale:"us"});$(this).formatNumber({format:"#,###.00", locale:"us"});$("#indicator_montant_"+row_nbr).parseNumber({format:"#,###.00", locale:"us"});$("#indicator_montant_"+row_nbr).formatNumber({format:"#,###.00", locale:"us"});$("input[name=\"indicator_montant[]\"]").each(function(i){if(!isNaN($("#indicator_verified_value_"+i).val().replace(/,/g,"")) && $("#indicator_verified_value_"+i).val()!=""){attrib_tot += parseFloat($("#indicator_verified_value_"+i).val().replace(/,/g,""));}if(!isNaN($("#indicator_verified_value_"+i).val().replace(/,/g,"")) && !isNaN($("#indicator_tarif_"+i).val().replace(/,/g,"")) && $("#indicator_tarif_"+i).val()!=""){available_tot += parseFloat($("#indicator_tarif_"+i).val().replace(/,/g,""));}$("#attributed_tot").val(attrib_tot);$("#available_tot").val(available_tot);$("#datafile_ttl").val(Math.round(((attrib_tot/available_tot)*100)*100)/100);$("#available_tot").parseNumber({format:"#,###.00", locale:"us"});$("#available_tot").formatNumber({format:"#,###.00", locale:"us"});$("#datafile_ttl").parseNumber({format:"#,###.00", locale:"us"});$("#datafile_ttl").formatNumber({format:"#,###.00", locale:"us"});});}});});function savefile(){if(parseFloat($("#datafile_total").val().replace(/,/g,"")) == parseFloat($("#datafile_ttl").val().replace(/,/g,""))){$("input[name=\"indicator_montant[]\"]").each(function(i){$("#indicator_montant_"+i).removeAttr("disabled");$("#indicator_tarif_"+i).removeAttr("disabled");});document.getElementById("frm_step_two").action="'.base_url().'datafiles/save";document.getElementById("frm_step_two").submit();}else{alert("'.$ci->lang->line('datafile_check_totals').'");return false;}}
</script>
<script>    
    $(function(){
