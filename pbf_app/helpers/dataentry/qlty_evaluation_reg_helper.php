...
function qlty_evaluation_reg($params){
	...
	$lang=$ci->config->item('language_abbr');
	  ...
	  LEFT JOIN pbf_lookups ON pbf_lookups.lookup_id=pbf_indicatorsfileypes.indicator_category_id
		   WHERE pbf_indicatorsfileypes.filetype_id='".$params['filetype_id']."' AND (LAST_DAY('".$params['datafile_year']."-".$params['datafile_month']."-1') BETWEEN pbf_indicatorsfileypes.use_from AND pbf_indicatorsfileypes.use_to) AND pbf_indicatorstranslations.indicator_language ='".$lang."' ";
        //echo $sql;
       // $details = $ci->datafiles_mdl->get_datafile_details($params['datafile_id'],$params['filetype_id'],$params['entity_id'],$params['datafile_month'],$params['datafile_year']);
		$sql_append=" ORDER BY pbf_indicatorsfileypes.order";
		
		$details1 = $ci->db->query($sql.$sql_append)->result_array();
		
		if(empty($details1)){//verification si il ya des indicateurs pour cette periode
       ...
       $sql_categorie="select * from  pbf_indicatorcategories ORDER BY category_order ASC";
			$categories = $ci->db->query($sql_categorie)->result_array();
        ...
        $sql_append=" AND pbf_indicatorsfileypes.indicator_category_id=".$categories[$key]['category_id']." ORDER BY pbf_indicatorsfileypes.order";
		
		$details = $ci->db->query($sql.$sql_append)->result_array();
        ...
        foreach ($details AS $detail){
         ...
         if(!empty($details)){
         		$cat_pourcentage=0;
         	 foreach ($details as $k2=>$row2) {
         	 	
				$cat_pourcentage+=$row2['indicator_tarif'];
         	 }
         	
         
        $ci->table->add_row(
              		array('data'=>''),
					 array('data' =>"<br/><b><u>".$categories[$key]['category_title']."</u> ($cat_pourcentage )</b>", 'colspan' => 4)
				)	;   
	$results = $details;
	//print_test($results);
