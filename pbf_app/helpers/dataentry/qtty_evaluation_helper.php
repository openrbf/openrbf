...
';
	  ...
	  AND pbf_indicatorstarif.indicatortarif_year='".$params['datafile_year']."' AND ((pbf_indicatorstarif.indicatortarif_entity_id=pbf_entities.entity_id AND pbf_indicatorstarif.indicatortarif_geozone_id=pbf_entities.entity_geozone_id) AND (pbf_indicatorstarif.indicatortarif_entity_type_id=pbf_entities.entity_type AND pbf_indicatorstarif.indicatortarif_entity_class_id=pbf_entities.entity_class)) AND pbf_indicatorstarif.indicatortarif_filetype_id=pbf_indicatorsfileypes.filetype_id)
		   WHERE pbf_indicatorsfileypes.filetype_id='".$params['filetype_id']."' AND (LAST_DAY('".$params['datafile_year']."-".$params['datafile_month']."-1') BETWEEN pbf_indicatorsfileypes.use_from AND pbf_indicatorsfileypes.use_to) AND pbf_indicatorstranslations.indicator_language ='".$lang."' ORDER BY pbf_indicatorsfileypes.order ,indicator_use_indigence_bonus";
		...
		$ci->table->set_template($table_tmpl);
                ...
                $ci->table->set_heading(array(	'#',$ci->lang->line('datafile_indicator'),
                  ...
                  $bm=1;
						...
						if($entity_group==2){
							$l='a';
						}elseif($entity_group==3){
							$l='b';
						}elseif($entity_group==4){
							$l='c';
						}else{
							$l='-';
						}
                ...
                foreach ($details as $k=>$row) {
                	...
                	if($row['datafiledetail_id']){
						...
						//$row['indicator_tarif']=number_format($row['indicator_tarif']/10,0);
						//$row['indicator_tarif']=str_replace(",", '', $row['indicator_tarif']);
						//$row['indicator_tarif']=str_replace(".", '', $row['indicator_tarif']);
						//$row['indicator_tarif']=$row['indicator_tarif']*10;
                		...
                		if($row['indicator_use_indigence_bonus']){
                			...
                			/*
							if($entity_group==2){//T2
															$row['indicator_tarif']=$row['indicator_tarif']+$row['bonus_indigent'];
														}elseif($entity_group==3){//T3
															$row['indicator_tarif']=$row['indicator_tarif']+($row['bonus_indigent']*1.25);
														}elseif($entity_group==4){//T4
															$row['indicator_tarif']=$row['indicator_tarif']+($row['bonus_indigent']*0.3);
														}*/
				$indigent_a_10pourcent=array(24,341,47,48,49,50,51,52);
				if($entity_group==4 AND in_array($row['indicator_id'], $indigent_a_10pourcent)){
					
					$indigence_bonus=10;
				}
							
                $row['indicator_tarif']=$row['indicator_tarif']+($row['bonus_indigent']*($indigence_bonus / 100));
                    ...
                    if($row['indicator_use_indigence_bonus']==1 AND $entity_group==1){
                    	
                    }else{
                    $ci->table->add_row(	
                                        //array('data' => $k+1, 'align' => 'right'),
                                        array('data' => $num, 'align' => 'right'),
