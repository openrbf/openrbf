...
class Translator extends CI_Controller {
	...
	function index() {
		if ($this->_configured ()) {
			...
			$this->masterModulePath = $this->langDir . '/' . $this->masterLang . '/' . $this->langModule . '_lang.php';
			$this->slaveModulePath = $this->langDir . '/' . $this->slaveLang . '/' . $this->langModule . '_lang.php';
			...
			if ($this->validated && $this->input->post ( 'ConfirmSaveLang' )) {
				if ($this->_translation_saved ( $result )) {
					/*
					 * $this->data['saved_data'] = $result; $this->data['page'] = 'translator/confirmLangSaved'; $this->data['mod_title']['mod_title'] = 'Changes Saved'; $this->data['mod_title']['dashboard/']=$this->pbf->rec_op_icon('close');
					 */
					$this->session->set_flashdata ( array (
							'mod_clss' => 'success',
							'mod_msg' => 'Changes Saved successfully' 
					) );
				} else {
					/*
					 * $this->data['page'] = 'translator/saveFailed'; $this->data['mod_title']['mod_title'] = 'Failure'; $this->data['mod_title']['dashboard/']=$this->pbf->rec_op_icon('close');
					 */
					$this->session->set_flashdata ( array (
							'mod_clss' => 'errormsg',
							'mod_msg' => 'Failure saving translation' 
					) );
				}
				redirect ( 'translator/' );
			} elseif ($this->validated && $this->input->post ( 'SaveLang' )) {
				$this->data ['page'] = 'translator/saveLang';
				$this->data ['mod_title'] ['mod_title'] = 'Confirm';
				$this->data ['mod_title'] ['dashboard/'] = $this->pbf->rec_op_icon ( 'close' );
			} else {
				$this->data ['page'] = 'translator/translateLang';
				$this->data ['mod_title'] ['mod_title'] = 'Translate ' . $this->langModule;
				$this->data ['mod_title'] ['dashboard/'] = $this->pbf->rec_op_icon ( 'close' );
			}
		}
	...
	function _configured() {
		...
		if (! $this->langDir) {
			...
			$this->data ['page'] = 'translator/selectLangDirectory';
			$this->data ['mod_title'] ['mod_title'] = "Select Language Directory";
			$this->data ['mod_title'] ['dashboard/'] = $this->pbf->rec_op_icon ( 'close' );
		} elseif (! $this->masterLang) {
			$this->data ['languages'] = $this->ListLanguages ();
			$this->data ['page'] = 'translator/selectMasterLang';
			$this->data ['mod_title'] ['mod_title'] = "Select Master Language";
			$this->data ['mod_title'] ['dashboard/'] = $this->pbf->rec_op_icon ( 'close' );
			...
			$this->data ['page'] = 'translator/selectSlaveLang';
			$this->data ['mod_title'] ['mod_title'] = "Select Slave Language";
			$this->data ['mod_title'] ['dashboard/'] = $this->pbf->rec_op_icon ( 'close' );
			...
			$this->data ['page'] = 'translator/selectModule';
			$this->data ['mod_title'] ['mod_title'] = "Select Module to Translate";
			$this->data ['mod_title'] ['dashboard/'] = $this->pbf->rec_op_icon ( 'close' );
		}
	...
	function _get_lang($line) {
		...
		/*
		 * TODO - This is no good if the string is a PHP expression e.g. 'Hello, ' + CONST + ' how\'s your world?' // Trim any encapsulating quote marks $line = trim( $line, '\'"' );
		 */
